...
def _via_metal_footprint_um(via_pad_um: float) -> float:
    """Actual M1/M2 square metal footprint produced by via_stack."""
...
def _below_cut_layer_for_metal(layer_tuple: tuple[int, int]) -> tuple[int, int] | None:
    ...
    mapping = {
        LAYER_M1: (33, 0),  # mcon under M1
        LAYER_M2: (35, 0),  # via1 under M2
    }
...
def _draw_route_segments(
    ...
    for i in range(len(points_um) - 1):
        ...
        if add_vias and i < len(points_um) - 2:
            ...
            via = c.add_ref(via_stack(con_size=(via_w, via_w), via_size=(via_w, via_w), x_range=(0, via_w), y_range=(0, via_w), metal_level=2))
            via.dcenter = p_next
